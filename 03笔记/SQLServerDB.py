...
# ***************************************************
# *
# * Description: Python操作SQLServer数据库辅助类(查询构造器)
# * Author: Raymond
# *
# ***************************************************
...
class SQLServerDB(object):
    ...
    def __init__(self, connection_str):
        ...
        self.body_fields = None
        self.body_keys = None
        self.tables = None
        self.tables_snippet = None
        self.condition_snippet = None
...
if __name__ == '__main__':
    connection_str = r'DRIVER={SQL Server Native Client 11.0};SERVER=DESKTOP-ULP591F;DATABASE=USStock;UID=sa;PWD=trui'
    db = SQLServerDB(connection_str)
    db.open()  # 打开数据库
    # db.execute("""
    #     CREATE TABLE [staffs] (
    ...
    # db.table("staffs").insert(staff_name="Raymond", staff_cardnum="1002", staff_reserved=0).execute()
    # # 插入一条记录
    #
    # db.table("staffs").update(staff_name="RaymondX", staff_cardnum="1012")\
    #     .where(DataCondition(("=", "AND"),staff_id=2)).execute()
    #
    # rs = db.table("staffs").select("staff_id", "staff_name").fetchall()
    # # 直接取出所有staff_id和staff_name
    # print(rs)
    #
    # rs = db.table("staffs").select("staff_name","staff_id")\
    #     .where(DataCondition(("=", "AND"), staff_name="Raymond")).fetchone()
    # # 取一条staff_id为1的staff_name
    # print(rs)
    #
    # rs = db.table("staffs").select("staff_name","staff_id")\
    #     .where(DataCondition(("=", "AND"), staff_name="Raymond"),
    #            DataCondition(("=", "AND"), staff_id=6)).fetchone()
